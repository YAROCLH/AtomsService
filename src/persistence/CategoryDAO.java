...
public class CategoryDAO {
		...
		public ArrayList<Category> getCategories(){
			...
			try {
				...
				pstmt = con.prepareStatement("SELECT IdCategory,Name,Description FROM "+SCHEMA+".Categories"); 
		...
		public boolean createCategory(Category category){
			try{
				...
				con=connector.CreateConnection();
				String Query="INSERT INTO "+SCHEMA+".CATEGORIES (NAME,DESCRIPTION,VIEWPOSITION,IMAGEBADGEURL,LEVEL01,LEVEL02,LEVEL03,LEVEL04) VALUES(?,?,?,?,?,?,?,?)"; 
		...
		public Category getCategory(int id){
			...
			try {
				con=connector.CreateConnection();
				pstmt = con.prepareStatement("SELECT * FROM "+SCHEMA+".CATEGORIES WHERE IDCATEGORY  = ?"); 
				pstmt.setInt(1,id);
				rs = pstmt.executeQuery();
				if(!rs.next()){	 
					category.setId(0);
		...
		public boolean setUpdateCategory(Category category){
				try{
					con=connector.CreateConnection();
					String Query="UPDATE "+SCHEMA+".CATEGORIES SET NAME=?,DESCRIPTION=?,LEVEL01=?,LEVEL02=?,LEVEL03=?,LEVEL04=?"
								+"WHERE IDCATEGORY=?"; 
					pstmt = con.prepareStatement(Query); 
					pstmt.setString(1,category.getName());
					pstmt.setString(2,category.getDescription());
